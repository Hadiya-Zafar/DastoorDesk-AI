  ...
  return (
    <Link href={`/chat/${department.id}`} className="block h-full group">
      <Card className="group relative h-full cursor-pointer overflow-hidden transition-all duration-500 hover:shadow-2xl hover:shadow-primary/20 bg-card/40 backdrop-blur-md border border-white/[0.08] hover:border-primary/50 hover:bg-card/60">
        {/* Intense primary purple spotlight effect */}
        <div className="absolute inset-0 bg-gradient-to-br from-primary/20 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700 mix-blend-screen" />
        
        <CardHeader className="relative pb-4 z-10">
          <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-xl bg-background/50 border border-white/10 shadow-inner group-hover:border-primary/60 group-hover:bg-primary/20 group-hover:scale-110 group-hover:shadow-lg group-hover:shadow-primary/40 transition-all duration-500">
          ...
          <div className="space-y-3">
            ...
            <ul className="space-y-2 mt-2">
              {department.examples.map((example, index) => (
                ...
                >
                  <span className="mt-1.5 h-1 w-1 shrink-0 rounded-full bg-primary/40 group-hover:bg-primary transition-colors shadow-sm shadow-primary/50" />
                  {example}
                </li>
              ))}
            </ul>

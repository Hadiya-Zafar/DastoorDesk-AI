...
export default function LogoIntro({ children }: { children: React.ReactNode }) {
  ...
  const [hoveredLang, setHoveredLang] = useState<"en" | "ur" | null>(null);
  const [activeLang, setActiveLang] = useState<"en" | "ur" | null>(null);
  const tickRef = useRef<SVGGElement>(null);
  const { setLanguage } = useLanguage();

  // ── Dismiss: fade out the intro and reveal the app ───────────────────────
  const dismiss = () => {
  ...
  // ── Pick language, then dismiss ──────────────────────────────────────────
  const pickLanguage = (lang: "en" | "ur") => {
    setSelectedLang(lang);
    setLanguage(lang);
    setTimeout(() => dismiss(), 400);
  };

  // ── Clicking the background (outside buttons) still advances ────────────
  const handleBackgroundClick = () => {
    // Selection is mandatory - do nothing on background click
  };
  ...
  useEffect(() => {
    ...
    // Show the language picker once the animation reaches its end-state
    const pickerTimer = setTimeout(() => setLangPickerVisible(true), 5200);
  ...
  return (
    <>
      {!hidden && (
        ...
        >
          ...
          <svg
            ...
            style={{ flexShrink: 0, overflow: "visible", position: "relative", zIndex: 10 }}
          >
            <defs>
          ...
          <div style={{ textAlign: "center", marginTop: 16, position: "relative", zIndex: 10 }}>
            ...
            {langPickerVisible && (
              <div
                ...
                style={{
                  marginTop: 40,
                  animation: "dd-picker-in 0.7s cubic-bezier(.16,1,.3,1) forwards",
                  display: "flex",
                  flexDirection: "column",
                  alignItems: "center",
                  gap: 20,
              ...
              >
                ...
                <div style={{ display: "flex", gap: 24, alignItems: "center" }}>
                  ...
                  <button
                    ...
                    onMouseLeave={() => { setHoveredLang(null); setActiveLang(null); }}
                    onMouseDown={() => setActiveLang("en")}
                    onMouseUp={() => setActiveLang(null)}
                    style={{
                      position: "relative",
                      display: "flex",
                      alignItems: "center",
                      ...
                      padding: "12px 0",
                      borderRadius: 999,
                      border: `1px solid ${selectedLang === "en" || hoveredLang === "en" ? primaryColor : "rgba(0,0,0,0.1)"}`,
                      background: selectedLang === "en" || activeLang === "en"
                        ? `linear-gradient(135deg, ${primaryColor}, ${accentColor})`
                        : hoveredLang === "en"
                          ? `linear-gradient(135deg, rgba(147, 51, 234, 0.1), rgba(147, 51, 234, 0.05))`
                          : "rgba(255, 255, 255, 0.4)",
                      color: selectedLang === "en" || activeLang === "en" ? "#fff" : "rgba(0,0,0,0.8)",
                      fontFamily: "sans-serif",
                      fontSize: 13,
                      fontWeight: 600,
                      ...
                      boxShadow: selectedLang === "en" || activeLang === "en"
                        ? `0 8px 25px -8px ${primaryColor}`
                        : hoveredLang === "en"
                          ? `0 8px 20px rgba(147, 51, 234, 0.15)`
                          : "0 4px 15px rgba(0,0,0,0.03)",
                      transform: activeLang === "en" ? "scale(0.96)" : hoveredLang === "en" ? "translateY(-2px)" : "translateY(0) scale(1)",
                  ...
                  {/* Urdu button */}
                  <button
                    onClick={() => pickLanguage("ur")}
                    onMouseEnter={() => setHoveredLang("ur")}
                    onMouseLeave={() => { setHoveredLang(null); setActiveLang(null); }}
                    onMouseDown={() => setActiveLang("ur")}
                    onMouseUp={() => setActiveLang(null)}
                    style={{
                      position: "relative",
                      display: "flex",
                      ...
                      padding: "10px 0",
                      borderRadius: 999,
                      border: `1px solid ${selectedLang === "ur" || hoveredLang === "ur" ? accentColor : "rgba(0,0,0,0.1)"}`,
                      background: selectedLang === "ur" || activeLang === "ur"
                        ? `linear-gradient(135deg, ${accentColor}, ${primaryColor})`
                        : hoveredLang === "ur"
                          ? `linear-gradient(135deg, rgba(147, 51, 234, 0.05), rgba(147, 51, 234, 0.1))`
                          : "rgba(255, 255, 255, 0.4)",
                      color: selectedLang === "ur" || activeLang === "ur" ? "#fff" : "rgba(0,0,0,0.8)",
                      fontFamily: "'Noto Nastaliq Urdu', 'Jameel Noori Nastaleeq', system-ui, sans-serif",
                      fontSize: 16,
                      fontWeight: 600,
                      cursor: "pointer",
                      backdropFilter: "blur(20px)",
                      boxShadow: selectedLang === "ur" || activeLang === "ur"
                        ? `0 8px 25px -8px ${accentColor}`
                        : hoveredLang === "ur"
                          ? `0 8px 20px rgba(147, 51, 234, 0.15)`
                          : "0 4px 15px rgba(0,0,0,0.03)",
                      transform: activeLang === "ur" ? "scale(0.96)" : hoveredLang === "ur" ? "translateY(-2px)" : "translateY(0) scale(1)",
                      transition: "all 0.3s cubic-bezier(0.16, 1, 0.3, 1)",
                      direction: "rtl",
                      outline: "none",
                    }}
      ...
      <div
        style={
          !hidden
            ? { height: "100vh", overflow: "hidden", pointerEvents: "none" }
